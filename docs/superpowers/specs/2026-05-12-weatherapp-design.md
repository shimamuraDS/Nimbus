...
│   └── components/
│       ├── Theme.qml           # 全局深色赛博朋克视觉主题定义(电光青/珊瑚/紫罗兰)
...
*   **默认触发（无提醒时间）**：遍历 `hourly_data` 中未来 1 小时内的逐小时预报，若 `weather` 字段非"晴"且非"多云"（即阴、雨、雪、雾、沙尘等），判定为异常。
*   **定时触发（有提醒时间）**：在用户设定的提醒时间点触发，不区分天气类型，所有天气均生成通知。
...
*   **`Config` (工具层)**：QSettings + config.ini 配置读写封装，管理自动/手动定位、开机自启动（Windows 注册表 `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`）、提醒时间列表等持久化设置。

### 2. QML组件树与明确的交互状态机

*   **`MainWindow`**：无边框窗口，宽乘高严格控制在 `< (Screen.width * Screen.height) / 12` 范围内。双层辉光边框（外层半透明白，内层微光），深色赛博朋克渐变背景。关联 `SystemTrayIcon` 实现驻留运行。
...
*   **`StackView` (页面导航)**：
    ...
    *   `SettingsView`（定位切换与提醒管理）：
        ...
        *   **开机自启动开关**：页面顶部"开机自启动"标签 + Toggle Switch 控件（电光青色），切换时平滑动画。
        *   **状态 A（自动定位中）**：
...
系统每分钟通过 `QTimer` 检测，根据是否设置提醒时间分为两种模式：

**模式 A：无提醒时间（默认模式）**
*   系统自动检测未来 1 小时内的逐小时预报。
*   当检测到非"晴"且非"多云"的天气（阴、雨、雪、雾、沙尘等）时，自动弹出 Windows 气泡通知。
*   同一天气事件只提醒一次，避免重复。

**模式 B：有提醒时间（定时模式）**
*   当系统时间匹配 `Alerts/Times` 中设定的提醒时间时触发。
*   不区分天气类型，所有天气（包括晴天）均生成提醒通知。
*   支持设置提前监测时长（`Alerts/AdvanceMinutes`），在触发后检查未来此时长范围内的天气。
*   通知内容通过 LLM（若启用）或固定模板生成。

### 3. Windows原生通知格式

由 `NotificationManager` 生成弹窗：

*   **通知标题**：若为预警命中，则展示预警名称（如"北京市海淀区发布大雾黄色预警"）。若为逐小时命中，则展示"天气提醒：未来1小时将有[天气]"（默认模式）或"天气提醒：未来X小时天气"（定时模式）。
*   **通知内容**：预警发布内容或逐小时天气预报。
