...
WeatherApp 是一款桌面天气提醒应用。该应用长期静默驻留于 Windows 任务栏系统托盘，支持开机自启动，提供当日逐小时天气时间线、未来 7 天及过去 7 天的早晚天气、气温与湿度展示。应用采用双重预警触发机制，结合腾讯位置服务的逐小时预报与官方实时灾害预警，在用户设定的特定时间点通过 Windows 原生通知弹窗进行天气提醒（除晴天外所有天气均触发提醒，同时告知当前天气和即将出现的异常天气）。

界面采用**深色赛博朋克（Dark Cyberpunk）**设计风格，电光青/珊瑚/紫罗兰三色强调，玻璃态卡片叠加在深邃暗色渐变背景上，配合方向感知的页面过渡动画（300ms OutQuint 缓动），提供现代化的视觉体验。
